...
//          new BufferedReader(new InputStreamReader(System.in)); 
        ...
        BufferedReader inFromServerSend = 
          new BufferedReader(new InputStreamReader(clientSocketSend.getInputStream())); 
        ...
        BufferedReader inFromServerRecv = 
          new BufferedReader(new InputStreamReader(clientSocketRecv.getInputStream())); 
        ...
        //while(true) {
             ...
             outToServerSend.writeBytes(sendRegSentence); 
             outToServerSend.flush();
             String responseSendRegSentence = inFromServerSend.readLine(); 
             ...
             outToServerRecv.writeBytes(recvRegSentence); 
...
//     String outSentence; 
     ...
     CLSocketThreadSend (String userName,Socket connectionSocketSend, BufferedReader inFromServer, DataOutputStream outToServer) {
...
//                   String userInput =stringBuffer.toString();
                   ...
                   if(userInput.trim().startsWith("@") && userInput.trim().contains(":") ){
                        String msg=userInput.trim().substring(userInput.indexOf(":")+1, userInput.length());
                        ...
                        outToServer.writeBytes(outSentence); 
                        outToServer.flush();
                        inSentence = inFromServer.readLine(); 
...
//     String outSentence; 
     ...
     CLSocketThreadRead (String userName, Socket connectionSocketRecv, BufferedReader inFromServer, DataOutputStream outToServer) {
            this.userName=userName;
         //this.connectionSocketSend = connectionSocketSend;
        this.connectionSocketRecv = connectionSocketRecv;
        this.inFromServer = inFromServer;
     ...
     public void run() {
	   ...
	   try {
	           ...
	           inSentence = inFromServer.readLine(); 
                   ...
                   if(inSentence!=null){
                        if(inSentence.trim().startsWith("FORWARD")){
                             String senderName=inSentence.substring(7, inSentence.length());
                             String msgSentence="";
                             String contentLengthSentence = inFromServer.readLine(); 
                             if(contentLengthSentence.startsWith("Content-length:")){
                                ...
                                try{
                                    ...
                                    char cbuf[]=new char[contLength+1];
                                     inFromServer.read(cbuf, 0, contLength+1);
